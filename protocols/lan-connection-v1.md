...
另有只读 [MCU 更新检查扩展](mcu-update-v1.md)，实际烧录请求仍被 controller 拒绝。
...
启用 MCU 更新处理器时，握手的 capabilities 额外包含 `mcu_update_check` 和
`mcu_update_status`；这表示能够查询，不代表可以烧录或 Go 服务已经就绪。
