    ...
    public class Composite { 
            ...
            for (Footprint.Pad pad : pads) {
                if (Math.abs(pad.getRotation() % 90) > eps) {
                    if (compositingMethod.isEnforced()) { 
                        throw new Exception(org.openpnp.Translations.format("Local.644c9ebb45a4b4ac", (pkg.getId()), (pad.getName())));
                    }
                }
        ...
        public void interpret() throws Exception {
            ...
            if(centerWeights==0) { // divide by zero ahead!
                throw new Exception(org.openpnp.Translations.format("Local.607e1ce78e564000", (pkg.getId())));
            }
            if(angleWeights==0) {
                throw new Exception(org.openpnp.Translations.format("Local.78e3b6682e9e654a", (pkg.getId())));
            }
