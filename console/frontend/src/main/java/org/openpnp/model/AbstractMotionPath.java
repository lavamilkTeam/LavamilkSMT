            ...
            for (int axis = 0; axis < profiles.length; axis++) {
                if (prevProfiles == null) {
                    if (profiles[axis].v[0] != 0) {
                        throw new Exception(org.openpnp.Translations.format("Local.3e51375d03781fb0", (title), (axis)));
                    }
                    if (!profiles[axis].isConstantAcceleration() && profiles[axis].a[0] != 0) {
                        throw new Exception(org.openpnp.Translations.format("Local.5292492474a4b11e", (title), (axis)));
                    }
                }
                else {
                    if ( MotionProfile.mismatch(profiles[axis].s[0], prevProfiles[axis].s[segments], sErr)) {
                        throw new Exception(org.openpnp.Translations.format("Local.db09e875acb7b1ff", (title), (axis), (i)));
                    }
                    if ( MotionProfile.mismatch(profiles[axis].v[0], prevProfiles[axis].v[segments], vErr)) {
                        throw new Exception(org.openpnp.Translations.format("Local.bebc62ce4a7791af", (title), (axis), (i)));
                    }
                    if (!profiles[axis].isConstantAcceleration() 
                            &&  MotionProfile.mismatch(profiles[axis].a[0], prevProfiles[axis].a[segments], aErr)) {
                        throw new Exception(org.openpnp.Translations.format("Local.837228256e947698", (title), (axis), (i)));
                    }
                }
            }
            // Next.
