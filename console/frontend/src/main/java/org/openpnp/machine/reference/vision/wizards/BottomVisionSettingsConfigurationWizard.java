...
public class BottomVisionSettingsConfigurationWizard extends AbstractConfigurationWizard {
    ...
    private void createUi() {
        ...
        usedIn = new JLabel(org.openpnp.Translations.getString("Local.dc937b59892604f5"));
            ...
            UiUtils.messageBoxOnException(() -> {
                if (settingsHolder != null && bottomVision.getParentHolder(settingsHolder) != null) {
                    if (visionSettings.getUsedBottomVisionIn().size() == 1 
                        ...
                        throw new Exception(org.openpnp.Translations.format("Local.148ff19d30ea8d41", (settingsHolder.getShortName())));
                    }
        ...
        btnGeneralizeSettings.addActionListener((e) -> {
            UiUtils.messageBoxOnException(() -> {
                ...
                if (list.size() == 0) {
                    throw new Exception(org.openpnp.Translations.format("Local.baa7a3371ecce8ca", (subjects), (settingsHolder.getClass().getSimpleName()), (settingsHolder.getShortName())));
                }
                int result = JOptionPane.showConfirmDialog(getTopLevelAncestor(),
                        org.openpnp.Translations.format("Local.811c50298350cd76", (new AbstractVisionSettings.ListConverter(false).convertForward(list))), null,
        ...
        resetButton.addActionListener(e -> {
            UiUtils.messageBoxOnException(() -> {
                int result = JOptionPane.showConfirmDialog(getTopLevelAncestor(),
                        org.openpnp.Translations.getString("Local.bad820a182ac7b10"), null,
                        JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE);
                if (result == JOptionPane.YES_OPTION) {
        ...
        comboBoxPreRotate = new org.openpnp.gui.components.LocalizedComboBox(ReferenceBottomVision.PreRotateUsage.values());
        ...
        comboBoxMaxRotation = new org.openpnp.gui.components.LocalizedComboBox(ReferenceBottomVision.MaxRotation.values());
        ...
        comboBoxCheckPartSizeMethod = new org.openpnp.gui.components.LocalizedComboBox(ReferenceBottomVision.PartSizeCheckMethod.values());
        ...
        pipelinePanel = new PipelineControls() {
            ...
            public void resetPipeline() throws Exception {
                int result = JOptionPane.showConfirmDialog(getTopLevelAncestor(),
                        org.openpnp.Translations.getString("Local.7b84817dab60936c"), null,
                        JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE);
                if (result == JOptionPane.YES_OPTION) {
                    UiUtils.messageBoxOnException(() -> {
                        applyAction.actionPerformed(null);
    ...
    private void pipelineConfiguration(CvPipeline pipeline, Map<String, Object> pipelineParameterAssignments, boolean edit) 
        ...
        if (pkg == null) {
            throw new Exception(org.openpnp.Translations.format("Local.575a3a84a476f2b2", (nozzle.getName())));
        }
        NozzleTip nt = nozzle.getNozzleTip();
        if (nt == null) {
            throw new Exception(org.openpnp.Translations.format("Local.0eff2462368c4505", (nozzle.getName())));
        }
        if (! pkg.getCompatibleNozzleTips().contains(nt)) {
            throw new Exception(org.openpnp.Translations.format("Local.629a2bde73f86df0", (nt.getName()), (nozzle.getName()), (pkg.getId()), (packageSource)));
        }
    ...
    public Nozzle getNozzleWithPart() throws Exception {
        ...
        Part part = nozzle.getPart(); 
        if (part == null) {
            throw new Exception(org.openpnp.Translations.format("Local.dad1715c02e0fb5a", (nozzle.getName())));
        }
        if (settingsHolder instanceof Part 
                && part != this.settingsHolder) {
            throw new Exception(org.openpnp.Translations.format("Local.161c9441562bab69", (part.getId()), (nozzle.getName())));
        }
        else if (settingsHolder instanceof Package 
                && part.getPackage() != this.settingsHolder) {
            throw new Exception(org.openpnp.Translations.format("Local.48550ca819b36d5c", (part.getPackage().getId()), (nozzle.getName())));
        }
        if (bottomVision == null) {
            throw new Exception(org.openpnp.Translations.format("Local.1d7b4494b263af22", (visionSettings.getName())));
        }
        if (bottomVision.getInheritedVisionSettings(part) != visionSettings) {
            throw new Exception(org.openpnp.Translations.format("Local.5ea581a24954ff45", (part.getId()), (nozzle.getName())));
        }
        return nozzle;
    }
