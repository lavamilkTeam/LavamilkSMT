...
public class ReferencePushPullFeeder extends ReferenceFeeder {
    ...
    public void assertCalibrated(boolean tapeFeed) throws Exception {
        if (getHole1Location().convertToUnits(LengthUnit.Millimeters).getLinearDistanceTo(getHole2Location()) < 3) {
            throw new Exception(org.openpnp.Translations.format("Local.57bc500eea4d161e", (getName())));
        }
        if ((visionOffset == null && calibrationTrigger != CalibrationTrigger.None)
            ...
            if (visionOffset == null) {
                // no lock obtained
                throw new Exception(String.format(org.openpnp.Translations.getString("Local.0fdece39d1db40a8"), getName()));
            }
        }
    }
    ...
    public void feed(Nozzle nozzle) throws Exception {
        ...
        if (actuator == null) {
            throw new Exception(String.format(org.openpnp.Translations.getString("Local.c6440c2edbdcd6d8"),
                    getName()));
        }
    ...
    public void ensureCameraZ(Camera camera, boolean setZ) throws Exception {
        if (camera.isUnitsPerPixelAtZCalibrated()
            ...
            throw new Exception(org.openpnp.Translations.format("Local.70a34632161bdea6", (getName())));
        }
    ...
    public void smartClone(Part compatiblePart, 
        ...
        if (templateFeeder == null) {
            if (compatiblePart == null) {
                throw new Exception(org.openpnp.Translations.format("Local.041e1634d5eaf3b2", (getName()))); 
            }
            else {
                throw new Exception(org.openpnp.Translations.format("Local.0f43a8eac9e2b107", (getName()), (compatiblePart.getId())));
            }
        }
    ...
    protected void triggerOcrAction(SimpleOcr.OcrModel ocrModel, OcrWrongPartAction ocrAction, boolean ocrStop,
        ...
        if (currentPart == null) {
            ...
            if (ocrAction == OcrWrongPartAction.SwapFeeders) {
                if (otherFeeder == null) {
                    throw new Exception(org.openpnp.Translations.format("Local.8fda4e9ff47f5289", (ocrPart.getId()), (getName())));
                }
            ...
            if (ocrStop) {
                throw new Exception(org.openpnp.Translations.format("Local.fcfabb7eee4c4cd1", (getName()), (currentPart.getId()), (ocrPart.getId()), (ocrAction.toString())));
            }
            else if (report != null) {
        ...
        if (closestFeeder != null) {
            ...
            if (isSnapToAxis()) {
                if (Math.abs(rowUnit.getX()) > rowLocationToleranceMm && Math.abs(rowUnit.getY()) <= rowLocationToleranceMm) {
                    ...
                    throw new Exception(org.openpnp.Translations.format("Local.f2ec7b162a3ce0c2", (closestFeeder.getName()), (closestFeeder.getPart().getId())));
                }
            }
        }
    ...
    protected void setOcrDetectedPart(Part ocrPart, boolean clone) throws Exception {
        if (isUsedAsTemplate()) {
            if (!compatiblePartPackages(ocrPart, getPart())) {
                throw new Exception(org.openpnp.Translations.format("Local.dc40f666da180db2", (getName())));
            }
            setPart(ocrPart);
        }
        else {
    ...
    public void performOcrOnAllFeeders(OcrWrongPartAction ocrAction, boolean ocrStop, StringBuilder report) throws Exception { 
        ...
        if (ocrFeederList.size() == 0) {
            throw new Exception(org.openpnp.Translations.getString("Local.e9afd21785de1919"));
        }
        // Now bulk-OCR. 
    ...
    public void performOcr(OcrWrongPartAction ocrAction, boolean ocrStop, StringBuilder report) throws Exception {
        if (getOcrRegion() == null) {
            throw new Exception(org.openpnp.Translations.format("Local.4bb43f13c967d444", (getName())));
        }
        Camera camera = getCamera();
    ...
    public PropertySheet[] getPropertySheets() {
        return new PropertySheet[] {
                ...
                new PropertySheetWizardAdapter(new ReferencePushPullMotionConfigurationWizard(this), org.openpnp.Translations.format("Local.a7b530108e98aa90")),
        };
    }
