...
public class ReferenceActuator extends AbstractActuator implements HeadMountable {
    ...
    protected void tryActuateBoolean(Machine machine, boolean value, boolean deferred) {
        try {
            ...
            UiUtils.messageBoxOnExceptionLater(() -> { 
                throw new Exception(org.openpnp.Translations.format("Local.c64365d9a17b6b67", (getName())), e);
            });
        }
    }
    ...
    public PropertySheet[] getPropertySheets() {
        ...
        if (getInterlockMonitor() != null) {
            propertySheets.add(new PropertySheetWizardAdapter(getInterlockMonitor().getConfigurationWizard(this), org.openpnp.Translations.format("Local.921adb00b4c7d11a")));
        }
        if (getValueType() == ActuatorValueType.Profile) {
            propertySheets.add(new PropertySheetWizardAdapter(getActuatorProfiles().getConfigurationWizard(this), org.openpnp.Translations.format("Local.535e52e4a2616ebe")));
        }
    ...
    public Action deleteAction = new AbstractAction(org.openpnp.Translations.getString("Local.db4250810366c511")) {
        {
