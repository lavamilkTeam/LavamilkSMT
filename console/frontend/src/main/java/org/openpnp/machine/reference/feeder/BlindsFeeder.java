...
public class BlindsFeeder extends ReferenceFeeder {
    ...
    public void feed(Nozzle nozzle) throws Exception {
        if (getFirstPocket() + getFeedCount() > getLastPocket()) {
            throw new Exception(org.openpnp.Translations.format("Local.23d7f9d951773f5a", (getName()), (getPart().getId())));
        }
        ...
        if (coverType == CoverType.BlindsCover) {
            if (coverActuation == CoverActuation.CheckOpen) {
                if (!isCoverOpenChecked()) {
                    ...
                    throw new Exception(org.openpnp.Translations.format("Local.e4f2d4a7691cb430", (getName()), (getPart().getName())));
                }
            }
    ...
    public class FindFeatures {
        ...
        public FindFeatures invoke() throws Exception {
            ...
            try {
                ...
                throw new Exception(org.openpnp.Translations.format("Local.eb6256b4edfc1f93", (results)));
            }
    ...
    public void findPocketsAndCenterline(Camera camera) throws Exception {
        ...
        if (!(fiducial1Location.isInitialized() && fiducial2Location.isInitialized() && fiducial3Location.isInitialized())) {
            throw new Exception(org.openpnp.Translations.format("Local.6c41aab3dbbbb286", (getName())));
        }
        ...
        if (coverType == CoverType.BlindsCover) {
            ...
            try (CvPipeline pipeline = getCvPipeline(camera, true, getOcrAction())) {
                ...
                if (Double.isNaN(findFeaturesResults.getPocketCenterlineMm())) {
                    throw new Exception(org.openpnp.Translations.format("Local.27c4036a13f47a87", (getName())));
                }

                if (Double.isNaN(findFeaturesResults.getPocketPitchMm())) {
                    throw new Exception(org.openpnp.Translations.format("Local.98abbb893e9ef53b", (getName())));
                }
                // TODO: validate pitch against known EIA pitch values {2, 4, 8, 12, 16...}

                if (Double.isNaN(findFeaturesResults.getPocketSizeMm())) {
                    throw new Exception(org.openpnp.Translations.format("Local.acd30759dd6e9f98", (getName())));
                }
    ...
    public void ensureCameraZ(Camera camera) throws Exception {
        if (camera.isUnitsPerPixelAtZCalibrated()
            ...
            throw new Exception(org.openpnp.Translations.format("Local.a3b90ddaefb8d40a", (getName())));
        }
    ...
    public void triggerOcrAction(OcrModel detectedOcrModel, OcrAction ocrAction) throws Exception {
        ...
        if (currentPart == null) {
            ...
            if (ocrAction == OcrAction.ChangePart) {
                ...
                throw new Exception(org.openpnp.Translations.format("Local.26e096a645923134", (getName()), (currentPart.getId()), (ocrPart.getId())));
            }
        }
        else if (ocrPart == null) {
            if (ocrAction == OcrAction.ChangePart
                    || ocrAction == OcrAction.CheckCorrect) {
                throw new Exception(org.openpnp.Translations.format("Local.afd2281f1c523676", (getName()), (currentPart.getId())));
            }
        }
    }
    ...
    public void performOcr(Camera camera, OcrAction ocrAction) throws Exception {
        ...
        try (CvPipeline pipeline = getCvPipeline(camera, true, ocrAction)) {
            ...
            OcrModel detectedOcrModel = findFeaturesResults.detectedOcrModel;
            if (detectedOcrModel == null) {
                throw new Exception(org.openpnp.Translations.format("Local.8d330a3918d7e37c", (getName())));
            }
            Logger.trace("OCR text "+detectedOcrModel.getText());
    ...
    public void findCoverPosition(Camera camera) throws Exception {
        try (CvPipeline pipeline = getCvPipeline(camera, true, OcrAction.None)) {
            ...
            if (Double.isNaN(findFeaturesResults.getPocketPositionMm())) {
                throw new Exception(org.openpnp.Translations.format("Local.877b87f0f4242ae5", (getName())));
            }
    ...
    public void calibrateCoverEdges()  throws Exception {
        if (coverType != CoverType.BlindsCover) {
            throw new Exception(org.openpnp.Translations.format("Local.79f3870f78f655ed", (getName())));
        }
        Camera camera = getCamera();
    ...
    private Location locateFiducial(Camera camera, Location location) throws Exception {
        if (!location.isInitialized()) {
            throw new Exception(org.openpnp.Translations.format("Local.6ecca84a2d574bb7", (getName())));
        }
        ...
        try (CvPipeline pipeline = getCvPipeline(camera, true, OcrAction.None)) {
            ...
            for (int i = 0; i < fidLocMaxPasses; i++) {
                ...
                if (fiducials.isEmpty()) {
                    throw new Exception(org.openpnp.Translations.format("Local.358b98d770374973", (getName())));
                }
                // Convert location.
    ...
    public void calibrateFeederLocations() throws Exception {
        if (!isCalibrating()) {
            ...
            if ( !Configuration.get().getMachine().isHomed() ) {
                throw new Exception(org.openpnp.Translations.format("Local.fe7e595ba8031c44", (getName())));
            }
    ...
    public static void actuateAllFeederCovers(Nozzle preferredNozzle, boolean openState) throws Exception  {
        ...
        if (feederList.size() == 0) {
            throw new Exception(org.openpnp.Translations.format("Local.c85007fc2bb46785", ((openState ? "open." : "close."))));
        }
    ...
    public static NozzleAndTipForPushing getNozzleAndTipForPushing(Nozzle preferredNozzle, boolean loadNozzleTipIfNeeded) throws Exception {
        ...
        if (loadNozzleTipIfNeeded) {
            ...
            throw new Exception(org.openpnp.Translations.getString("Local.62cac37323326f01"));
        }
        // None compatible.
    ...
    public void actuateCover(Nozzle preferredNozzle, boolean openState, boolean loadNozzleTipIfNeeded, boolean restoreNozzleTip) throws Exception {
        if (coverType == CoverType.NoCover) {
            throw new Exception(org.openpnp.Translations.format("Local.1df79e08607b4d3d", (getName())));
        }
        else {
            if (location.getZ() == 0.0) {
                throw new Exception(org.openpnp.Translations.format("Local.6b8a0ef3c571b1c2", (getName())));
            }
            ...
            if (nozzleTip == null) {
                throw new Exception(org.openpnp.Translations.format("Local.b7d1735f6207cec3", (getName())));
            }
            Length nozzleTipDiameter = nozzleTip.getDiameterLow(); 
            if (nozzleTipDiameter.getValue() == 0.) {
                throw new Exception(org.openpnp.Translations.format("Local.e3a0375aac9ed2ff", (getName()), (nozzleTip.getId())));
            }
    ...
    public PropertySheet[] getPropertySheets() {
        return new PropertySheet[] {
                new PropertySheetWizardAdapter(getConfigurationWizard(), org.openpnp.Translations.format("Local.b332c3492d5eb10a")),
                new PropertySheetWizardAdapter(new BlindsFeederArrayConfigurationWizard(this), org.openpnp.Translations.format("Local.b8f4c47b136bd4fb")),
        };
    }
