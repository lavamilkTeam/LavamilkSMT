...
public class ReferenceLinearTransformAxis extends AbstractTransformedAxis {
    ...
    public double[] getLinearTransform() throws Exception {
        if (factorX != 0.0 && inputAxisX == null) {
            throw new Exception(org.openpnp.Translations.format("Local.d8650b7447261bf9", (getName()))); 
        }
        if (factorY != 0.0 && inputAxisY == null) {
            throw new Exception(org.openpnp.Translations.format("Local.297b2d99565ac862", (getName()))); 
        }
        if (factorZ != 0.0 && inputAxisZ == null) {
            throw new Exception(org.openpnp.Translations.format("Local.c4557b17f4a7cc64", (getName()))); 
        }
        if (factorRotation != 0.0 && inputAxisRotation == null) {
            throw new Exception(org.openpnp.Translations.format("Local.c076353493226cb7", (getName()))); 
        }
        return new double[] { 
        ...
        for (Axis axis : location.getAxes()) {
            if (axis instanceof ReferenceLinearTransformAxis) {
                ...
                if (linearAxes[i] != null) {
                    throw new Exception(org.openpnp.Translations.format("Local.ebb0052bc87501ac", (linearAxes[i].getName()), (axis.getName())));
                }
    ...
    protected void consolidateInputAxes(Axis axis, AbstractAxis inputAxis, Axis.Type inputType,
        ...
        if (inputAxis != null) {
            if (inputAxes[j] != null && inputAxes[j] != inputAxis) {
                throw new Exception(org.openpnp.Translations.format("Local.09fab39917e6b4bd", (axis.getName()), (inputType)));
            }
