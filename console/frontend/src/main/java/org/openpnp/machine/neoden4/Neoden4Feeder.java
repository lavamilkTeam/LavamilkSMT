...
public class Neoden4Feeder extends ReferenceFeeder {
    ...
    public void feed(Nozzle nozzle) throws Exception {
        ...
        if (actuatorName == null) {
            throw new Exception(org.openpnp.Translations.getString("Local.9b34d5bbfc422156"));
        }
		...
		if (actuator == null) {
			throw new Exception(
					String.format(org.openpnp.Translations.getString("Local.d5b9a5dc80b9df3d"), actuatorName, head.getName()));
		}
    ...
    private Location getVisionOffsets(Head head, Location pickLocation) throws Exception {
        ...
        if (camera == null) {
            throw new Exception(org.openpnp.Translations.getString("Local.782c9909cfa95040"));
        }
        
        if (vision.getTemplateImage() == null) {
            throw new Exception(org.openpnp.Translations.getString("Local.6fa551429eacf15d"));
        }
        
        if (vision.getAreaOfInterest().getWidth() == 0 || vision.getAreaOfInterest().getHeight() == 0) {
            throw new Exception(org.openpnp.Translations.getString("Local.9abd4926da3904ae"));
        }
