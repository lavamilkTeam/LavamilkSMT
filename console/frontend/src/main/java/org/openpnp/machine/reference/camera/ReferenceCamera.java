...
public abstract class ReferenceCamera extends AbstractBroadcastingCamera implements HeadMountable {
    ...
    public void autoAdjustWhiteBalance(boolean averaged) throws Exception {
        ...
        if (lead < 32) {
            throw new Exception(org.openpnp.Translations.format("Local.5d6b84e162b28d65", (getName())));
        }
                ...
                if (level == levels) {
                    ...
                    if (level+1 >= levels) {
                        ...
                        if (colorMap[ch][level-1] > colorMap[ch][level+1]) {
                            throw new Exception(org.openpnp.Translations.format("Local.eb523f68fe7af360", (100*level/levels), (100*(level+1)/levels)));
                        }
                        else {
                            // Simply interpolate.
