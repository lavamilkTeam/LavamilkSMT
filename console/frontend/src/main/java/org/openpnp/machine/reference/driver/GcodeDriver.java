...
public class GcodeDriver extends AbstractReferenceDriver implements Named {
    ...
    public void home(Machine machine) throws Exception {
        ...
        if (homeCompleteRegex != null) {
            receiveResponses(homeCompleteRegex, timeout, (responses) -> { 
                throw new Exception(org.openpnp.Translations.getString("Local.d31abe11c0535a75")); 
            });
        }
            ...
            for (String variable : getAxisVariables((ReferenceMachine) machine)) {
                ...
                if (axis != null) {
                    if (hasVariable(command, variable)) {
                        ...
                        throw new Exception(org.openpnp.Translations.format("Local.d4550b8f1c83ec05", (variable)));
                    }
                }
                else {
                    command = substituteVariable(command, variable, null);
    ...
    public AxesLocation getReportedLocation(long timeout) throws Exception {
        ...
        if (command == null) {
            throw new Exception(org.openpnp.Translations.format("Local.643dd1151db5f1e6", (getName())));
        }
        if (getCommand(null, CommandType.POSITION_REPORT_REGEX) == null) {
            throw new Exception(org.openpnp.Translations.format("Local.ed5f1a3611030758", (getName())));
        }
        ...
        // Timeout expired.
        throw new Exception(org.openpnp.Translations.format("Local.d4413e9681f516cf", (getName()), (command)));
    }
    ...
    public void moveTo(HeadMountable hm, MoveToCommand move)
        ...
        if (isUsingLetterVariables() && isSupportingPreMove()) {
            throw new Exception(org.openpnp.Translations.format("Local.cda12bbf392344bf", (getName())));
        }
        ...
        if (command == null) {
            ...
            if (isSupportingPreMove()) {
                throw new Exception(org.openpnp.Translations.format("Local.540d92606f7c9e7f", (getName()), (hm.getClass().getSimpleName()), (hm.getName())));
            }
            else {
                throw new Exception(org.openpnp.Translations.format("Local.76136c92009c52cd", (getName())));
            }
        }
        if (hasVariable(command, "BacklashFeedRate")) {
            throw new Exception(org.openpnp.Translations.format("Local.5d12e8e6b1a44e55", (getName())));
        }
        ...
        for (String variable : getAxisVariables(machine)) {
            ...
            if (axis != null) {
                ...
                if (hasVariable(command, "BacklashOffset"+variable)) {
                    throw new Exception(org.openpnp.Translations.format("Local.0181cdb3e42e0bf0", (getName())));
                }
            ...
            if (isMotionPending()) {
                ...
                if (moveToCompleteRegex != null) {
                    receiveResponses(moveToCompleteRegex, completionType == CompletionType.WaitForStillstandIndefinitely ?
                            -1 : getTimeoutAtMachineSpeed(),
                            (responses) -> {
                        throw new Exception(org.openpnp.Translations.getString("Local.af6b87c260518925"));
                    });
                }
            }
    ...
    public String actuatorRead(Actuator actuator, Object parameter) throws Exception {
        ...
        if (command != null && regex != null) {
            ...
            List<Line> responses = receiveResponses(regex, timeoutMilliseconds, (r) -> {
                throw new Exception(String.format(org.openpnp.Translations.getString("Local.e640153a2d02c0b7"), actuator.getName()));
            });
            ...
            for (Line line : responses) {
                ...
                if (matcher.matches()) {
                    ...
                    try {
                        ...
                        throw new Exception(String.format(org.openpnp.Translations.getString("Local.841323cab5337d24"),
                                actuator.getName()), e);
                    }
                    catch (Exception e) {
                        throw new Exception(String.format(org.openpnp.Translations.getString("Local.e914861faa81223a"),
                                actuator.getName()), e);
                    }
                }
            }
            // This should not happen, as the regex is pre-matched in receiveResponses().
            throw new Exception(String.format(org.openpnp.Translations.getString("Local.f9af63c36ce3f5ad"), actuator.getName()));
        }
        else {
            throw new Exception(String.format(org.openpnp.Translations.getString("Local.7b0f19b62399ba90"), actuator.getName()));
        }
    }
    ...
    protected Line waitForConfirmation(String command, long timeout)
        ...
        // Timeout expired.
        throw new Exception(org.openpnp.Translations.format("Local.d4413e9681f516cf", (getCommunications().getConnectionName()), (command)));
    }
    ...
    protected void bailOnError() throws Exception {
        if (errorResponse != null) {
            ...
            errorResponse = null;
            throw new Exception(org.openpnp.Translations.format("Local.9252205d7e54bb38", (getCommunications().getConnectionName()), (error)));
        }
        if (readerThread == null || !readerThread.isAlive()) {
            throw new Exception(org.openpnp.Translations.format("Local.12d5e9915b82d1e3", (getCommunications().getConnectionName())));
        }
    }
    ...
    public String receiveSingleResponse(String regex) throws Exception {
        List<Line> responses = receiveResponses(regex, getTimeoutMilliseconds(), (r) -> {
            throw new Exception(String.format(org.openpnp.Translations.getString("Local.7904ceaec724844e"), regex));
        });
        if (responses == null) {
            return null;   
        }
