...
public class SimulationModeMachine extends ReferenceMachine {
    ...
    public PropertySheet[] getPropertySheets() {
        return Collect.concat(super.getPropertySheets(),
                new PropertySheet[] {
                        new PropertySheetWizardAdapter(new SimulationModeMachineConfigurationWizard(this), org.openpnp.Translations.format("Local.05ec7bc6df5dbfd4"))
                });
    }
                                    ...
                                    if ((Boolean)value == true) {
                                        ...
                                        if (checkPnP && !nozzle.getPart().getId().equals("TEST-OBJECT")) {
                                            if (!((ImageCamera) camera).isPickLocation(location, nozzle)) {
                                                throw new Exception(org.openpnp.Translations.format("Local.d15351eb3d77b76d", (nozzle.getName()), (nozzle.getPart().getId())));
                                            }
                                        }
                                        ...
                                        if (checkPnP && !nozzle.getPart().getId().equals("TEST-OBJECT")) {
                                            ...
                                            if (!((ImageCamera) camera).isPlaceLocation(location, nozzle)) {
                                                throw new Exception(org.openpnp.Translations.format("Local.322a4af1a335b513", (nozzle.getName()), (nozzle.getPart().getId())));
                                            }
                                        }
                                    }
                                }
