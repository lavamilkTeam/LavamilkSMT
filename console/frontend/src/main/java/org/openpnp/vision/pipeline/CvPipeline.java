...
public class CvPipeline implements AutoCloseable {
    ...
    public Result getExpectedResult(String name) throws Exception {
        if (name == null || name.trim().isEmpty()) {
            throw new Exception(org.openpnp.Translations.getString("Local.4af7713cac3e10ac"));
        }
        CvStage stage = getStage(name);
        if (stage == null) {
            throw new Exception(org.openpnp.Translations.format("Local.73019544bfd46630", (name)));
        }
        Result result = getResult(stage);
        if (result == null) {
            throw new Exception(org.openpnp.Translations.format("Local.0fb5eafbafb08474", (name)));
        }
    ...
    public void process() throws Exception {
        ...
        for (CvStage stage : stages) {
            ...
            try {
                if (!stage.isEnabled()) {
                    throw new Exception(String.format(org.openpnp.Translations.getString("Local.7cfa345193028129"), stage.getName()));
                }
