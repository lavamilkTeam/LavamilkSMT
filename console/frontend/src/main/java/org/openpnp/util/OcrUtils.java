    ...
    public static Part identifyDetectedPart(OcrModel detectedOcrModel, Feeder feeder) throws Exception {
        ...
        for (Part part : cfg.getParts()) {
            if (part.getId().equals(ocrText)) {
                ...
                if (ocrPart != null) {
                    // Uh-oh, ambiguous!
                    throw new Exception(org.openpnp.Translations.format("Local.4c139dab8b3572ab", (ocrText), (feeder.getName()), (ocrPart.getId()), (part.getId())));
                }
                ocrPart = part;
            }
        }
        if (ocrPart == null) {
            throw new Exception(org.openpnp.Translations.format("Local.bc5be150dfc8fc74", (feeder.getName()), (ocrText), (detectedOcrModel.getAvgScore())));
        }
        return ocrPart;
    }
}
