...
public abstract class CalibrateCameraProcess {
    ...
    public CalibrateCameraProcess(MainFrame mainFrame, CameraView cameraView, 
        ...
        if (setOfDifferentCalibrationHeights.size() < 2) {
            throw new Exception(org.openpnp.Translations.getString("Local.9d446a0cbc87dae6"));
        }
        
        if (detectionDiameters.size() != calibrationLocations.size()) {
            throw new Exception(org.openpnp.Translations.getString("Local.e5c2ea81c829a01e"));
        }
    ...
    private boolean initialAction() {
        ...
        if (isHeadMountedCamera) {
            ...
            setInstructionsAndProceedAction(org.openpnp.Translations.getString("Local.3cd849fae140d691"), 
                    ()->requestOperatorToAdjustDiameterAction(),
            ...
            setInstructionsAndProceedAction(org.openpnp.Translations.getString("Local.e1dc11c70d6b432e"),
                    ()->requestOperatorToCenterNozzleTipAction());
        }
        return true;
    }
    ...
    private boolean requestOperatorToAdjustDiameterAction() {
                    ...
                    try {
                        ...
                        keyPoints = pipeline.getExpectedResult(VisionUtils.PIPELINE_RESULTS_NAME)
                                .getExpectedListModel(KeyPoint.class, 
                                        new Exception(org.openpnp.Translations.getString("Local.2528cd20a88f00b6")));
        ...
        setInstructionsAndProceedAction(org.openpnp.Translations.getString("Local.db7ed86421d3ec64"), 
                ()->fiducialDiameterIsSetAction());
    ...
    private boolean estimateUnitsPerPixelAction() {
        ...
        swingWorker = new SwingWorker<Void, String>() {
            ...
            @Override
            protected Void doInBackground() throws Exception {
                publish(org.openpnp.Translations.getString("Local.41122e3620f26c0e"));
                ...
                publish(org.openpnp.Translations.getString("Local.823c7a6e534ce64f"));
                Logger.trace(org.openpnp.Translations.getString("Local.823c7a6e534ce64f"));
            ...
            protected void done() {
                ...
                if (!cameraWalker.isReadyToWalk()) {
                    if (!isCancelled()) {
                        UiUtils.showError(new Exception(org.openpnp.Translations.getString("Local.61a45cf0bf934bca")));
                    }
                    cleanUpWhenCancelled();
                    return;
                }
        ...
        swingWorker = new SwingWorker<Void, String>() {
            ...
            protected Void doInBackground() throws Exception {
                ...
                for (int iLine=0; iLine<numberOfLines; iLine++) {
                    publish(String.format(org.openpnp.Translations.getString("Local.10e3c634e9a8a37f"), 
                    ...
                    try {
                        ...
                        if (errorCount > props.maxErrorCount) {
                            Logger.trace(org.openpnp.Translations.getString("Local.9a8823d91c028356"));
                            return null;
                        }
                    }
                    if (isCancelled()) {
                ...
                while (!done) {
                    ...
                    for (int iLine=0; iLine<numberOfLines; iLine++) {
                        publish(String.format(org.openpnp.Translations.getString("Local.15639fa9e0c78835"), 
                                iLine+1, numberOfLines,
                                loopCount+1,
                                calibrationHeightIndex+1, numberOfCalibrationHeights));
                        try {
            ...
            protected void done()  
            {
                ...
                if (errorCount > props.maxErrorCount) {
                    UiUtils.showError(new Exception(org.openpnp.Translations.getString("Local.2fbd0fe700e66a6c")));
                    cleanUpWhenCancelled();
                    return;
                }
                ...
                if (calibrationHeightIndex >= numberOfCalibrationHeights) {
                    mainFrame.showInstructions(org.openpnp.Translations.format("Local.7332f253e5852acb"), 
                            org.openpnp.Translations.format("Local.c9feaa8d84152e10"), 
                            true, false,  org.openpnp.Translations.format("Local.1ff57a29d7c9d11b"), cancelActionListener, proceedActionListener);
    ...
    private boolean requestOperatorToCenterNozzleTipAction() {
        ...
        setInstructionsAndProceedAction(org.openpnp.Translations.getString("Local.302c13774d601871"), 
                ()->captureCentralLocationAction());
        return true;
    }
    ...
    private boolean requestOperatorToVerifyNozzleTipIsCenteredAction() {
        ...
        setInstructionsAndProceedAction(org.openpnp.Translations.getString("Local.969d1916c844190d"), 
                ()->captureVerifiedCentralLocationAction());
        return true;
    }
    ...
    private Point findAveragedFiducialPoint(Point expectedPoint) {
        ...
        do {
            ...
            if (observedPoint == null) {
                Logger.trace(org.openpnp.Translations.getString("Local.574be72bf90fb749"));
                return null;
            }
    ...
    private void setInstructionsAndProceedAction(String instructions, 
        ...
        mainFrame.showInstructions(org.openpnp.Translations.format("Local.7332f253e5852acb"), 
                "<html><body>" + str + "</body></html>", 
                true, true,  org.openpnp.Translations.format("Local.1ff57a29d7c9d11b"), cancelActionListener, proceedActionListener);
    ...
    private final ActionListener cancelActionListener = new ActionListener() {
        @Override
        public void actionPerformed(ActionEvent e) {
            Logger.trace(org.openpnp.Translations.getString("Local.13b34def9f0f24bf"));
            cleanUpWhenCancelled();
        }
    };
