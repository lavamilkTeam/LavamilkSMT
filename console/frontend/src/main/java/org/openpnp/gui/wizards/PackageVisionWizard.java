...
public class PackageVisionWizard extends AbstractConfigurationWizard {
    ...
    public PackageVisionWizard(Package pkg) {
        ...
        unitsCombo = new org.openpnp.gui.components.LocalizedComboBox(LengthUnit.values());
    ...
    public final Action newAction = new AbstractAction() {
        ...
        public void actionPerformed(ActionEvent arg0) {
            ...
            while ((name = JOptionPane.showInputDialog(getTopLevelAncestor(),
                    org.openpnp.Translations.getString("Local.2596ee8e586e7f36"))) != null) {
                name = name.trim();
                if (name.isEmpty()) {
                    break;
                }
        ...
        UiUtils.messageBoxOnException(() -> {
            try{
                if (!footprint.getPads().isEmpty()) {
                    int ret = JOptionPane.showConfirmDialog(getTopLevelAncestor(),
                            org.openpnp.Translations.getString("Local.ce52261379bd1d23"),
                            org.openpnp.Translations.getString("Local.603b667e8321c82c"), JOptionPane.YES_NO_OPTION);
                    if (ret != JOptionPane.YES_OPTION) {
                        return; // Abort.
                    }
