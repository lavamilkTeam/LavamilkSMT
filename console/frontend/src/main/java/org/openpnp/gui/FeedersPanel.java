...
public class FeedersPanel extends JPanel implements WizardContainer {
    ...
    public FeedersPanel(Configuration configuration, MainFrame mainFrame) {
        ...
        JComboBox<Type> feedOptionsComboBox = new org.openpnp.gui.components.LocalizedComboBox(ReferenceFeeder.FeedOptions.values());
        JComboBox<Type> priorityComboBox = new org.openpnp.gui.components.LocalizedComboBox(ReferenceFeeder.Priority.values());
        ...
        if (feederConfigurationIsDirty && (priorFeeder != null)) {
            ...
            try {
                selection = JOptionPane.showConfirmDialog(null,
                        org.openpnp.Translations.format("Local.1ff35d9e3abf1e2c", (priorFeeder.getName())),
                        org.openpnp.Translations.getString("Local.6a7d6a38f88d4ad7"),
    ...
    private void newFeeder(Part part) {
        ...
        if (Configuration.get().getParts().size() == 0) {
            MessageBoxes.errorBox(getTopLevelAncestor(), org.openpnp.Translations.getString("Local.54a0e8c17ebb21a1"),
                    org.openpnp.Translations.getString("Local.a1258feab8e80603"));
            return;
        }
        ...
        try {
            ...
            MessageBoxes.errorBox(JOptionPane.getFrameForComponent(FeedersPanel.this),
                    org.openpnp.Translations.getString("Local.323f25c39328b79b"), e);
        }
    }
    ...
    public static Nozzle feedFeeder(Feeder feeder) throws Exception {
        if (feeder.getPart() == null) {
            throw new Exception(org.openpnp.Translations.format("Local.34d6dc736f4d24e9", (feeder.getName())));
        }
    ...
    public static void pickFeeder(Feeder feeder) throws Exception, JobProcessorException {
        ...
        if (nozzle.isPartOffEnabled(Nozzle.PartOffStep.BeforePick)) {
            ...
            nozzle.moveToSafeZ();
            if(!nozzle.isPartOff()) {
                throw new JobProcessorException(nozzle, org.openpnp.Translations.getString("Local.134c483453f322a7"));
            }
        }
        ...
        if (nozzle.isPartOnEnabled(Nozzle.PartOnStep.AfterPick)) {
            if(!nozzle.isPartOn()) {
                throw new JobProcessorException(nozzle, org.openpnp.Translations.getString("Local.c48e9678ea31ee10"));
            }
        }
    ...
    protected static Nozzle getCompatibleNozzleAndTip(Feeder feeder, boolean allowNozzleTipChange) throws Exception {
        if (feeder.getPart() == null) {
            throw new Exception(org.openpnp.Translations.getString("Local.56c5b94baab9beb6"));
        }
        ...
        if (nozzle.getNozzleTip() == null || 
            ...
            String errMsg = "";
            if (nozzle.getNozzleTip() == null) {
                errMsg += org.openpnp.Translations.getString("Local.3b07cf21e08da511")+nozzle.getName()+". ";
            }
            else {
                errMsg += "Nozzle "+nozzle.getName()+org.openpnp.Translations.getString("Local.b974d2f12960e1e5")+
                        nozzle.getNozzleTip().getName()+org.openpnp.Translations.getString("Local.7a2012447e1a13ba")+packag.getId()+". ";
                if (nozzle.getPart() != null) {
                    errMsg += org.openpnp.Translations.getString("Local.584e7c99ae1b3a4d")+nozzle.getPart().getId()+" loaded. "; 
                }
            }
            if (altNozzle != null) {
                errMsg += org.openpnp.Translations.getString("Local.210bb835e23540fb")+altNozzle.getName()+org.openpnp.Translations.getString("Local.66ba18cdc590529b")+altNozzle.getNozzleTip().getName()+" loaded. ";
            }
            else if (allowNozzleTipChange && !nozzle.isNozzleTipChangedOnManualFeed()) { 
                errMsg += org.openpnp.Translations.getString("Local.c3f850948ce1851c");
            }
            errMsg += org.openpnp.Translations.getString("Local.51605cf62e3a4d35");
